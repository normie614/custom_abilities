...
Action OnWeaponCanSwitchTo(int client, int weapon) //TODO: Check what happens if both are active
{
    Action result;

    result = Knives_OnWeaponCanSwitchTo(client, weapon);

    if (result != Plugin_Continue)
        return result;

    result = Shield_OnWeaponCanSwitchTo(client, weapon);
...
public Action OnTakeDamageAlive(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	if (StrEqual(classname, "entity_medigun_shield")) // Prevent the shield from doing massive damage.
	{
		if (ShouldBlockProjectileMultiHit(inflictor, victim))
		{
			damage = 0.0;
	...
	switch (index)
	{
		...
		case 998: // The Vaccinator
		{
			if (cvarVaccinatorAbility.BoolValue && abilityState[client].secondaryCooldown < GetGameTime() && buttons & IN_ATTACK3)
			{
				...
				abilityState[client].lockUntil = GetGameTime() + 11.0; // Prevent from using other weapons during the shield.
